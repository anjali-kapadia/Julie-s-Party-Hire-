...
def hire_b():
    ...
    receipt_number = tkinter.Label(frame_1, text="Receipt Number :")
    ...
    def validate_inputs():
        ...
        def hire_receipt():
            window_2 = tkinter.Tk()
            window_2.title("Hire Reciept - Julie's Party Hire")
            window_2.geometry('350x350')

            frame_2 = tkinter.Frame(window_2,width=325, height=350)
            frame_2.pack(padx = 10, pady=10)

            tkinter.Label(frame_2, text=f"Receipt Number: #{receipt_number}").pack(pady=5)
            tkinter.Label(frame_2, text=f"First Name: {first_name}").pack(pady=5)
            tkinter.Label(frame_2, text=f"Last Name: {last_name}").pack(pady=5)
            tkinter.Label(frame_2, text=f"Item(s) Hired: {item_hire}").pack(pady=5)


        # Insert data into the hire_info table
        valid = check_empty(first_name) and check_length(first_name) and check_input(first_name) and check_empty(last_name) and check_length(last_name) and check_input(last_name) and check_empty(item_hire) and check_empty(quantity_hire)
        if valid:
            first_name = first_name_entry.get()
            last_name = last_name_entry.get()
            item_hire = item_hire_combo.get()
            quantity_hire = quantity_hire_spinbox.get()
            receipt_number = randInt
            

            #Show message that hire information is added
            #Ask user if they want to hire more items
            question_box = tkinter.messagebox.askquestion("Success","Hire information added successfully.\nWould you like to hire more items?")
            #If user clicks "yes" they can hire more items as their previous selections are cleared
            if question_box == "yes":
                reset()
                return
            #if user clicks "no" receipt will be printed
            else:
                hire_receipt()
